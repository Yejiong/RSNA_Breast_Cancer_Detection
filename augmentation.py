...
def do_random_contrast(image, mul=[-0.3,0.3],pow=[-0.5,0.5],add=[-0.2,0.2]):
    #image = image.astype(np.float32)/255
    u = np.random.choice(3)
    if u==0:
        m = np.random.uniform(*mul) #-0.3,0.3)
        image = image*(1+m)
    if u==1:
        m = np.random.uniform(*pow) #-0.5,0.5)
        image = image**(1+m)
    if u==2:
        m = np.random.uniform(*add) #-0.2,0.2)
        image = image + m
...
def run_xxx():
	...
	dx0, dy0, dx1, dy1, dx2, dy2, dx3, dy3 = sx0, sy0, sx1, sy1, sx2, sy2, sx3, sy3

	src = np.random.uniform(0,1,(sh,sw,sc))
	dst = np.random.uniform(0,1,(dh,dw,dc))


	matrix = cv2.getPerspectiveTransform(
		np.array([sx0, sy0, sx1, sy1, sx2, sy2, sx3, sy3,],np.float32).reshape(4,2),
		np.array([dx0, dy0, dx1, dy1, dx2, dy2, dx3, dy3,],np.float32).reshape(4,2),
	)
